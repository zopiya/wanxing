...
The soul and form layers answer *what the rules are*. This file answers a different question: **given a design request, what procedure ensures the rules actually land in the output.** When philosophy and convention collide, do not improvise — walk the three layers in [tracks.md](./tracks.md).
...
- [ ] Logo（如存在）是否遵循 [`wenxin/brand.md`](./soul/brand.md)：红底白线、2.5px 线宽、`rx="4"`、不做暗色模式变色？
